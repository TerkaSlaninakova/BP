...
from utils import  read_audio, write_data, plot, create_inputs, create_targets, timestamp, Log
from train import Net
from generate import Generator

TRAIN_ITERATIONS = 7000
TARGET_COST = 0.1
PATH_TO_WAV = 'data/bed.wav'
OUTPUT_DIR='data/'
LEANING_RATE = 0.001
GPU_FRACTION = 0.3
SAMPLE_RATE = 32000
...
def get_arguments():
    ...
    parser.add_argument('--plot', type=bool, default=PLOT, help='Bool to decide whether to plot output data' + str(PLOT))
...
if __name__ == '__main__':
    ...
    plot(parser.output_dir + 'in_'+timestamp()+'.png', data, parser.sample_rate, parser.plot)
    ...
    model.train(inputs, targets, parser.target_cost, parser.train_iters)

    gen = Generator(model)
    inputs_ = inputs[:, 0:1, 0]
    final_predictions = gen.run(inputs_, data.shape[0])

    plot(parser.output_dir + 'out_'+timestamp()+'.png', final_predictions, parser.sample_rate, parser.plot)
